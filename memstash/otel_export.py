...
memstash stays local-first by default — every call already lives in SQLite. But
...
or any OTLP backend, *without* memstash depending on a server.
...
`pip install 'memstash[otel]'`. If it's missing or disabled, `export()` is a
...
def oi_attributes(trace: dict) -> dict:
    """Map a memstash trace to OpenInference LLM span attributes."""
    ...
    attrs = {
        ...
        "memstash.cost_usd": trace.get("cost_usd"),
        "memstash.kind": trace.get("kind", "chat"),
    }
...
def _get_tracer():
    ...
    try:
        ...
        _TRACER = provider.get_tracer("memstash")
        return _TRACER
