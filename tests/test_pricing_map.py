...
from recall.pricing import _candidates, estimate_cost, price_of
...
def test_inline_override(monkeypatch):
    monkeypatch.setenv("RECALL_PRICING", json.dumps({"my-model": {"input": 1.0, "output": 2.0}}))
...
def test_file_override(monkeypatch):
    ...
    monkeypatch.setenv("RECALL_PRICING_FILE", str(p))
...
def test_inline_beats_file(monkeypatch):
    ...
    monkeypatch.setenv("RECALL_PRICING_FILE", str(p))
    monkeypatch.setenv("RECALL_PRICING", json.dumps({"m": {"input": 9.0, "output": 0.0}}))
