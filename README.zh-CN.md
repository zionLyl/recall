...
- 🤖 **自动记忆** —— 从对话里捕获你的偏好(中英文)
- ♻️ **冲突消解**(opt-in)—— ADD/UPDATE/DELETE/NOOP,事实始终保持最新
- ⏳ **生命周期 + 双时态** —— 软遗忘、新近度排序、"某个时间点我知道什么"
- 🕸️ **graph-lite** —— SQLite 里的实体关系(不用图数据库)
- 📄 **文档摄入** —— 丢进 `.md`/`.txt`/`.pdf` 即可检索
- 🌊 **流式 + 交互式 REPL** —— `memstash chat`,多轮
- 📊 **可观测** —— 每调用 token/成本/延迟,逐轮**调用树**,评测
- 📡 **接管现有应用** —— 用 `memstash.instrument()` 收编 LangChain/OpenAI-SDK 的调用
- 💰 **日预算** —— 警告 + 可选硬上限
- 🔌 **22 个 provider** —— 云、国产云、快速推理、本地
- 🔗 **MCP server** —— 任意 agent(Claude Desktop/Code、Cursor)读写你的记忆
- 🏠 **100% 本地** —— 无账号、无服务器、无遥测
