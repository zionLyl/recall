...
recall stays local-first by default — every call already lives in SQLite. But
...
or any OTLP backend, *without* recall depending on a server.
...
def oi_attributes(trace: dict) -> dict:
    """Map a recall trace to OpenInference LLM span attributes."""
    ...
    attrs = {
        ...
        "recall.cost_usd": trace.get("cost_usd"),
        "recall.kind": trace.get("kind", "chat"),
    }
...
def _get_tracer():
    ...
    try:
        ...
        _TRACER = provider.get_tracer("recall")
        return _TRACER
