...
class MemoryEngine:
    ...
    def remember(self, content: str, tags: Optional[list[str]] = None) -> int:
        ...
        if model is not None:
            ...
            embedding = _pack(vec)
        return self.store.add_memory(content, tags=tags, embedding=embedding)
    ...
    def recall(self, query: str, limit: int = 5) -> list[Memory]:
        ...
        scored: list[Memory] = []
        for mid, content, tags, blob, created in self.store.all_memories_with_embeddings():
            if not blob:
                continue
            score = _cosine(qvec, _unpack(blob))
            scored.append(Memory(id=mid, content=content, tags=tags, created_at=created, score=score))
        scored.sort(key=lambda m: m.score, reverse=True)
        top = [m for m in scored if m.score > 0.15][:limit]
        # If embeddings missed everything, fall back to keyword.
        return top or self.store.keyword_search(query, limit=limit)
